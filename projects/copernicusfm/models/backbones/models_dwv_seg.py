...
class CopernicusFMViT(nn.Module):
    ...
    def forward_features(self, x, meta_info, key, wave_list, bandwidth, language_embed, input_mode, kernel_size=None):
        ...
        # resize pos embed
        token_h = (input_h + 2 - kernel_size) // kernel_size + 1
        token_w = (input_w + 2 - kernel_size) // kernel_size + 1
        num_patches_sqrt_origin = int(math.sqrt(self.num_patches))
        pos_embed = resize_abs_pos_embed(self.pos_embed, (token_h, token_w), (num_patches_sqrt_origin,num_patches_sqrt_origin), num_prefix_tokens=1)
