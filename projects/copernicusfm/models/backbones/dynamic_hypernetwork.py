...
class Dynamic_MLP_OFA_variable(nn.Module):
    ...
    def __init__(self, wv_planes, inter_dim=128, kernel_size=3, embed_dim=1024, option='language'):
        ...
        if self.option == 'spectrum':
            ...
            # Language embeddings must be loaded by the outer OpenMMLab module
            # from config and passed into forward.
    ...
    def forward(self, key, img_feat, wvs, bandwidths, language_embed, kernel_size=None):
        ...
        if self.option == 'spectrum':
            ...
            if language_embed is None:
                raise RuntimeError(
                    'language_embed must be provided from the config when '
                    'Dynamic_MLP_OFA_variable is used with option="language"')
